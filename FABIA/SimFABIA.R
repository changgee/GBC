...
SimFABIA_DEF <- function(R,seed,p,n,type,param,overlap,L,Lmax,batch=0)
{
  CE = rep(0,R)
  FP = rep(0,R)
  FN = rep(0,R)
  ...
  list(p=p,n=n,type=type,param=param,overlap=overlap,L=L,Lmax=Lmax,S=S,CE=CE,FP=FP,FN=FN,SEN=SEN,SPE=SPE,MCC=MCC,CS=CS,Lhat=Lhat)
}
